"""Tests for the fast-feature HGBR ablation pipeline."""
...
from scripts.run_stage_c_ablation_fast_hgbr import main as ablation_main
...
def test_stage_c_ablation_fast_hgbr_main_writes_outputs(tmp_path: Path):
    ...
    assert rc == 0
    assert (out_dir / "ablation_cv_metrics.csv").exists()
    assert (out_dir / "ablation_cv_permutation_importance.csv").exists()
    ...
    assert summary["model_name"] == "HistGradientBoostingRegressor"
    assert summary["feature_set_name"] == "fast_only"
